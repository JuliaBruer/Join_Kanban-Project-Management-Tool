...
function getSubtaskHTML(subtask, functionName, index, taskID) {
  return /*html*/ `
  <div id="subtask-container${index}" class="subtask-container">
    <label onchange="${functionName}(${index}, ${taskID} )" id="subtask-label${index}" class="subtask-label">
      <input id="overlayCheckbox${index}" type="checkbox"> ${subtask.name}
    </label>
      <div class="subtask-icons">
        <img onclick="editSubtask(${index})" src="../assets/img/icons/edit.png">
        <img onclick="deleteSubtask(${index})" src="../assets/img/icons/delete.png">
      </div>
    </div>
  `;
}
