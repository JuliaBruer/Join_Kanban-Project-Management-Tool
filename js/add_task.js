...
/**
 * this function opens the category dropdown and overlay for selecting categories
 */
function openCategoryAndOverlay() {
  document.getElementById("closeOverlayCategory").classList.remove("d-none");
...
/**
 * this function closes the overlay and resets the category dropdown to its initial state
 */
function closeOverlayCategory() {
  const categoryBox = document.getElementById("category");
  categoryBox.innerHTML = getTaskCategotryHTML();
  document.getElementById("closeOverlayCategory").classList.add("d-none");
}
...
/**
 * this function opens the user dropdown and overlay for selecting users
 */
function openUserAndOverlay() {
  document.getElementById("closeOverlayUser").classList.remove("d-none");
  toggleUserPicker();
}
...
/**
 * this function closes the overlay and resets the user dropdown to its initial state
 */
function closeOverlayUser() {
  const userBox = document.getElementById("assigned-to");
  hideusers(userBox);
  document.getElementById("closeOverlayUser").classList.add("d-none");
}
...
/**
 * this function replaces the img of the selected priority element with another image
 *
...
/**
 * this function deletes a subtask based on its index and removes it from the array of current subtasks
 *
 * @param {number} index - The index of the subtask to be deleted.
 */
function deleteSubtask(index) {
  const container = document.getElementById(`subtask-container${index}`);
  if (container) {
    container.remove();
...
/**
 * this function copies the label text of a subtask to the subtask input field for editing and then deletes the original subtask
 *
 * @param {number} index - The index of the subtask to be edited and deleted.
 */
function editSubtask(index) {
  const labelText = document.getElementById(`subtask-label${index}`).textContent.trim();
  document.getElementById("subtask-input").value = labelText;
  deleteSubtask(index);
}
