...
/**
 * this function renders tasks on the board based on the provided searchedTasks
 *
 * @param {Array} searchedTasks - An array of tasks to render. If null, it will render all tasks.
 */
...
/**
 * this function checks and renders an empty task area for each specified area if it has no child elements
 */
function findEmptyAreas() {
  ...
  for (let i = 0; i < areas.length; i++) {
    ...
    if (area.childElementCount === 0) {
      area.innerHTML = `
        <div class="empty-area">no tasks available</div>
      `;
    }
  }
}
